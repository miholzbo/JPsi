...
namespace Analysis {
    ...
    StatusCode JpsiFinder::initialize() {
        ...
        // Get the Trigger Decicion Tool
        CHECK( m_tdt.retrieve());
//        m_chaingroup = m_tdt->getChainGroup("HLT_mu.*_bJpsi_Trkloose");
        m_chaingroup = m_tdt->getChainGroup(".*");
        ATH_MSG_INFO("Triggers in ChainGroup: " << m_chaingroup->getListOfTriggers().size());
        for (std::string triggername : m_chaingroup->getListOfTriggers()){
            ATH_MSG_INFO("    " << triggername);
        }

        // Get the Trigger Matching Tool
        m_tmt.setTypeAndName("Trig::MatchingTool/TriggerMatchingTool");
        CHECK(m_tmt.retrieve());
...
//                if ( fabs(getPt(TP))<m_trkThresholdPt ) continue;
        ...
        ATH_MSG_DEBUG("Number of candidates before selection: " << jpsiCandidates.size());

        // Pair-wise selections
        std::vector<JpsiCandidate>::iterator jpsiItr;


        // (0) Require Trigger Decision & Matching
        if (m_mutrk){
            const Trig::ChainGroup* cg = m_tdt->getChainGroup("HLT_mu.*_bJpsi_Trkloose");
            // Trigger Decision
            if (!m_tdt->isPassed(cg)) {
                jpsiCandidates.clear();
                ATH_MSG_DEBUG("Trigger has not been fired!");
            }
            // Trigger Matching
            int index(0);
            ...
            for (jpsiItr = jpsiCandidates.begin(); jpsiItr!=jpsiCandidates.end(); ++jpsiItr,++index) {
                myParticles.clear();
                myParticles.push_back((*jpsiItr).muon1);
                if (  m_tmt->match( myParticles, "HLT_mu4_bJpsi_Trkloose", 0.005) ) listToDelete.push_back(index); // HLT_mu4_bJpsi_Trkloose
            }
        ...
        if (m_mutrk){
            ...
            ATH_MSG_DEBUG("Number of candidates after cuts on Tag Muon and z0 impact parameter: " << jpsiCandidates.size());
        ...
        if(tracksToKeep.size()>=1 && muons.size()>=1){
            for(trkItr=tracksToKeep.begin();trkItr<tracksToKeep.end();trkItr++){
                for(muItr=muons.begin();muItr<muons.end();muItr++){
                    ...
                    pair.pairType = MUTRK;
                    if ((*muItr)->muonType() == xAOD::Muon::Combined ){
                        pair.muonTypes = CT;
                    }
                    else{
                        pair.muonTypes = TT;
