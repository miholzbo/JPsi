...
${CMTROOT}/${CMTBIN}/cmt.exe cleanup -sh -pack=MyPackage -version=MyPackage-00-00-00 -path=/afs/cern.ch/user/m/miholzbo/athena/JPsi  $* >${cmtMyPackagetempfile}
if test $? != 0 ; then
  echo >&2 "${CMTROOT}/${CMTBIN}/cmt.exe cleanup -sh -pack=MyPackage -version=MyPackage-00-00-00 -path=/afs/cern.ch/user/m/miholzbo/athena/JPsi  $* >${cmtMyPackagetempfile}"
  cmtcleanupstatus=2
